...
if [ "$CLANG18" = true ] && [ "$HOMEBREW" = true ]; then
  echo "Error: -c/--clang18 and -h/--homebrew options are mutually exclusive."
  exit 1
fi

if [ "$CLANG18" = false ] || [ "$HOMEBREW" = true ]; then
...
echo "| Clang18 (-c, --clang18)        | $(printf "%-5s" $([ "$CLANG18" = true ] && echo "Yes" || echo "No")) |"
